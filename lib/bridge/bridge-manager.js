...
export class BridgeManager {
  ...
  _isOwner(platform, userId, agentId) {
    ...
    const prefs = this.engine.getPreferences();
    const ownerDict = prefs.bridge?.owner || {};
    // Check per-agent key first, then legacy global key
    const perAgentKey = agentId ? `${platform}:${agentId}` : null;
    const ownerId = (perAgentKey && ownerDict[perAgentKey]) || ownerDict[platform] || null;
  ...
  async sendProactive(text, targetAgentId) {
    ...
    for (const [, entry] of this._platforms) {
      ...
      // Look up owner with per-agent key first, then legacy global
      const entryAgentId = entry.agentId;
      const perAgentKey = entryAgentId ? `${platform}:${entryAgentId}` : null;
      const ownerId = (perAgentKey && ownerDict[perAgentKey]) || ownerDict[platform] || null;
      if (!ownerId) continue;
