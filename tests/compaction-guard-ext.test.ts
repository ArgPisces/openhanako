...
describe("CompactionGuardExtension", () => {
  ...
  describe("L3: session_before_compact preemptive hard truncate", () => {
    ...
    it("builds lossy-local compaction before model auth and removes tool protocol blocks", async () => {
      pi = createMockPi();
      const branchEntries = [
        {
          id: "old-user",
          ...
          message: { role: "user", content: [{ type: "text", text: "keep this original text" }] },
        },
        {
          id: "old-assistant",
          parentId: "old-user",
        ...
        },
        {
          ...
          message: { role: "toolResult", content: [{ type: "text", text: "tool bytes disappear" }] },
        },
        {
          id: "kept-user",
          ...
          message: { role: "user", content: [{ type: "text", text: "recent tail" }] },
        },
      ];
      const getLossyLocalSummarySource = vi.fn(() => null);
      createCompactionGuardExtension({
        getCompactionMode: () => "lossy_local",
        getLossyLocalSummarySource,
        cacheCompactor,
      })(pi);
      const auth = vi.fn(async () => ({ ok: true, apiKey: "unused" }));
      ...
      expect(res?.compaction).toMatchObject({
        firstKeptEntryId: "kept-user",
        tokensBefore: 1234,
        details: {
          ...
          omitted: { toolResults: 1, toolCalls: 1 },
        },
      });
      expect(res.compaction.summary).toContain("keep this original text");
      expect(res.compaction.summary).toContain("assistant text stays");
      expect(res.compaction.summary).not.toContain("tool bytes disappear");
      expect(res.compaction.summary).not.toContain("### User\nrecent tail");
      expect(getLossyLocalSummarySource).toHaveBeenCalledWith(
